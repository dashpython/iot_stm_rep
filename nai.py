...
page_2_graph =dbc.Jumbotron([
    ...
    ],style={'maxHeight':"400px","overflowX":"scroll","overflowY":"scroll",'width':'600px'}),],style={"border":"2px black solid",'maxHeight':'500px','width':'600px','padding': '0px 0px 20px 0px'}),
    ],fluid=True)
...
def table(filtered_df):
    ...
    table_body=[
        ...
        html.Tbody(html.Tr([html.Td(dev[0]),html.Td(dev[1]),html.Td(dev[2]),html.Td(dev[3]),html.Td(dev[4])]))for dev in filtered_df]

      #  html.Thead(html.Tr([html.Th(col) for col in filtered_df.columns]))]
   
   # table_body=[html.Tbody(html.Tr([html.Td(filtered_df.iloc[i][col] for col in filtered_df.columns)]))]

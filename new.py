...
banner = html.Div(id ="banner",className ="banner",
    children=[html.Div(id="banner-logo",
        children = [html.Img(src=app.get_asset_url("smarttrack.png"), height="50px")],style={'display':'flex','flex-direction':'row','align-items': 'center','justify-content': 'flex-end'}),
                html.Div(id="banner-text",
                    children=[
                        html.H5("SMARTTRAK DASHBOARD")],style={'font-family': 'Open Sans Semi Bold','font-weight': '100','line-height': '1.2','font-size': '2rem','letter-spacing': '0.1rem','text-transform': 'uppercase'})],
                    style={"height": "fit-content","backgroundColor": "#1e2130","display": "flex",'flex-direction': 'row','align-items': 'center','justify-content': 'space-between','border-bottom':'1px solid #4B5460','padding':'1rem 8rem','width': '100%'})
...
app.layout = html.Div(id="big-app-container",
        children=[
            banner,row1,dcc.Location(id='url', refresh=False)],style={'padding':'1rem','display':'flex',"max-width": "100%",'max-height':'100%','flex-direction': 'column','align-items':'center','padding':'0 0.5rem'})    
...
page_2_graph =html.Div([html.A(dbc.Button("Home",color="success",className="mr-4"),href="/page-2"),
    ...
    ]),], style={"border":"2px black solid", "height":"calc(100vh - 10rem - 1px)"}),])])
...
daq.Indicator(label="DC Switch",color="green",value=True),])],
style={'width':'15rem','height':'4rem'}),
dbc.Card(dbc.Col([#html.H6("SPD Status"),
daq.Indicator(
label="SPD Status",
  color="green",
   value=True
...
dbc.Card(dbc.Col([html.H6("Power"),html.Div("0.000 KW")]))],style={'width':'30rem'})]),html.Table(stringtable()),]),dcc.Link(href='/page-16')])
#/*------------------------------------------------------------------------------------------------------------------*/

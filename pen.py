...
class DeviceModel(db.Model):
    ...
    def __init__(self,row_id,stamp,devId,SPA,TA):
        self.row_id = row_id
        self.stamp = stamp
        self.devId = devId
        self.SPA = SPA
...
def on_message(client, userdata, message):
  #global num
    ...
    try:
        ...
        x=["row_id","stamp"]
        y=[n,str(datetime.now()+ timedelta(minutes=330))]
        ...
        if  len(data)>2:
            sql = "INSERT INTO data ('%s','%s','%s','%s','%s') VALUES ('%s','%s','%s','%s','%s')" % (x[0],x[1],x[2],x[3],x[4],y[0],y[1],y[2],y[3],y[4])
...
def table(rows):
    ...
    #reading=reading+1
    table_header=[
        html.Thead(html.Tr([html.Th('row_id'),html.Th('stamp'),html.Th('devId'),html.Th('SPA') ,html.Th('TA')#, html.Th('motor status') ,
         ]))]
    table_body=[
...
if __name__=="__main__":
    print("main starts")
    db.create_all()
    app.run_server(debug=True,port=492)
    #mqtt1()
    #app1.run(debug=True)
    print("main end")

...
db.session.add(DeviceModel(stamp= str(datetime.now()+ timedelta(minutes=330)), devId='R1' ,SPA= 51.85, TA=0.0 ))
db.session.commit()
...
def table(rows):
    ...
    #reading=reading+1
    table_header=[
        html.Thead(html.Tr([html.Th('stamp'),html.Th('devId'),html.Th('SPA') ,html.Th('TA')#, html.Th('motor status') ,
         ]))]
    table_body=[
        html.Tbody(html.Tr([html.Td(dev[0]),html.Td(dev[1]),html.Td(dev[2]),html.Td(dev[3])]))for dev in rows]
